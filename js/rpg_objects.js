...
Game_Action.prototype.itemHit = function(target) {
    if (this.isPhysical()) {
        return this.item().successRate * 0.01 * this.subject().hit + this.subject().luk * 0.005;
    } else {
...
Game_Action.prototype.itemEva = function(target) {
    if (this.isPhysical()) {
        return target.eva + target.agi * 0.005;
...
Game_Action.prototype.itemCri = function(target) {
    ...
    if (this.subject().luk >= target.luk) {
        criRate = Math.pow(this.subject().luk / target.luk, 2);
    } else {
        criRate = this.subject().luk / target.luk;
    }
    return this.item().damage.critical ? this.subject().cri * (1 - target.cev) * criRate : 0;
};
...
Game_Actor.prototype.equipSlots = function() {
    ...
    // add accessory to enable 2 accessories equipment.
    slots.push($dataSystem.equipTypes.length - 1);
...
Game_Party.prototype.maxItems = function(item) {
    // âÒïúÉAÉCÉeÉÄÇÃÇ›èäéùêîÇ20å¬Ç…êßå¿
    if (item.id <= 20) {
        return 20;
    } else {
        return 99;
...
Game_Troop.LETTER_TABLE_FULL = [
    'Ôº°','Ôº¢','Ôº£','Ôº§','Ôº•','Ôº¶','Ôºß','Ôº®','Ôº©','Ôº™','Ôº´','Ôº¨','Ôº≠',
    'ÔºÆ','ÔºØ','Ôº∞','Ôº±','Ôº≤','Ôº≥','Ôº¥','Ôºµ','Ôº∂','Ôº∑','Ôº∏','Ôºπ','Ôº∫'
];
